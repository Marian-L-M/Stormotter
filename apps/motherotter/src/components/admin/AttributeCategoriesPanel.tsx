...
export function AttributeCategoriesPanel() {
  const categories = useAttributesStore((state) => state.categories)
  const addCategory = useAttributesStore((state) => state.addCategory)
  const updateCategory = useAttributesStore((state) => state.updateCategory)
  const removeCategory = useAttributesStore((state) => state.removeCategory)
    ...
    <details className="admin-attribute-categories-panel">
      ...
      <div className="admin-attribute-categories-body">
        <p className="field-hint">
          Group attributes by category for filtering in this list and in character editors.
        </p>
        {categories.length === 0 ? (
